...
<%
	...
	if (strPassword.equals("") || strEmail.equals("")) {
		...
		int intT=0;
		intT=data.getRowCount("owner WHERE ownerEmail='"+strEmail+"' AND password='"+strPassword+"'");
		if(intT>0){
		    session.setAttribute("email",strEmail);
		    response.sendRedirect("rent-listing.jsp");
		}else{
		        out.print("<script>alert('Incorrect Email or Password.Please try again.');document.location='host-login.html';</script>");
		        return;
		}
	}
%>
